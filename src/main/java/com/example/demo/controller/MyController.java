...
public class MyController {
    ...
    public Boolean friend(Integer userid, Integer user2) {
        List<Integer> friendsid = new ArrayList<>();
        List<Relationship> friends1 = relationshipRepository.findAllByFriend1id(userid);
        for (Relationship friendship : friends1) {
            friendsid.add(friendship.getFriend2id());
        }
        List<Relationship> friends2 = relationshipRepository.findAllByFriend2id(userid);
    ...
    public String membersite(Model model, @AuthenticationPrincipal MyUserDetails userdetails) {
        ...
        for (User member : members) {
            ...
            if (friend(userdetails.getId(), member.getId())) { member.setFreund(true);}
        }
    ...
    public String unfriend(@PathVariable(name="friendid") Integer friendid, @AuthenticationPrincipal MyUserDetails userdetails) {
        relationshipRepository.deleteRelationship(friendid, userdetails.getId());
